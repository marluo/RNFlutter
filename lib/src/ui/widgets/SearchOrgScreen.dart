...
class _SearchOrgScreenState extends State<SearchOrgScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(appBar:AppBar(
          title: Text('Sök efter en organisation'),
          actions: <Widget>[
            TextButton(
    ...
    hintText: widget.searchPlaceHolder
            ...
            CompanyButton(company:company, orgOrName: widget.orgOrName) else Text('wwee')],
        ),
      ));
  }
}
...
class CardListIncident extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return ClassListIncident(arguments: arguments, company: company, orgOrName: orgOrName);
  }
}
    ...
    return Container(child:Card(child:ListTile(
                        onTap: () {
                          Navigator.push(context,
                                  MaterialPageRoute(
      builder: (context) => NameScreen(org:company['orgname'], asd:arguments[0], db:arguments[1]),
    ));
                        },
                        title: Text(company['orgname']),
                        subtitle: Text(company['orgid']),
                        leading: Icon(
